...
class S_ASPP_SimAM(nn.Module):
    def __init__(self, dim_in, dim_out, rate=1, bn_mom=0.1):
        ...
        self.branch3 = DepthwiseSeparableConv2d(dim_in, dim_out, kernel_size=3, padding=12 * rate, dilation=12 * rate)
        self.branch4 = DepthwiseSeparableConv2d(dim_in, dim_out, kernel_size=3, padding=18 * rate, dilation=18 * rate)
...
class ASPP_FPN(nn.Module):
    ...
    def forward(self, x):
        ...
        for feature, lat_conv, output_conv in zip(x[1:], self.fpn_laterals[1:], self.fpn_outputs[1:]):
            # lat_features = self.s_aspp(lat_conv(feature))
        ...
        # change on 2023/7/8
        features_ca = self.ca(features)
        features_ca_sa = self.sa(features_ca)

        # 7/8的有涨点再试试下面这个
        # features_sa = self.sa(features)
        # # features = ca_feature * self.ca(features) + sa_feature * self.sa(features)
        # features = features_ca_sa * ca_feature + features_sa * sa_feature
        # return features
        return features_ca_sa
