...
class PAFPN_bottom_to_up(nn.Module):
    def __init__(self, in_channel_list, out_channel, cfg):
        ...
        # print("PAFPN_bottom_to_up_in_channel_list.type:", type(in_channel_list))
        # print("PAFPN_bottom_to_up_in_channel_list.length:", len(in_channel_list))
        # self.pa_fpn_features = in_channel_list
        # self.fpn = ASPP_FPN(in_channel_list, out_channel)
    ...
    # 自底向上部分
    def forward(self, x):
        # x = [x[f] for f in self.pa_fpn_features]
        # print('pa_fpn_x.type:', type(x))
        head_output = []  # 存放最终输出特征图
        # fpn_out = self.fpn(x)  # FPN操作,更改为x==fpn_outputs
        # print('------------FPN--PAFPN--------------')  # PAFPN操作分割线
        # print('x[-1].shape:', x[-1].shape)  # P3 32,256,80,108
        ...
        for i in range(1, len(x), 1):
            ...
            # pre_inner = paddle.concat([x[-1 - i], pre_concat], 1)  # concat
            # pre_inner = np.concatenate([x[-1 - i].cpu().detach().numpy(), pre_concat.cpu().detach().numpy()], axis=1)
            # pre_inner = torch.tensor(pre_inner).cuda()
...
class InstanceContextEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        pa_fpn_outputs = self.pa_fpn(fpn_outputs)
        # print("pa_fpn_outputs.type:", type(pa_fpn_outputs))  # list类型
        # 不确定pa_fpn的输出是否是P3、P4、P5
        ...
        features = [
                       pa_fpn_outputs[0]] + [F.interpolate(x, size, mode='bilinear', align_corners=False) for x in
                                             pa_fpn_outputs[1:]]
        # print("pre_fusion.len:", len(features))
